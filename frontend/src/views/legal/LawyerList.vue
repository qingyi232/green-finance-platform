<template>
  <div>
    <PageHeader title="律师列表" parent="法律服务" />
    <div class="page-container">
      ...
      <div class="grid">
        <div v-for="item in list" :key="item.id" class="grid-card eco-card" @click="onClick(item)">
          ...
          <div class="gc-stats">
            <div><strong>{{ item.years }}</strong><small>年执业</small></div>
            <div><strong>{{ item.caseCount }}</strong><small>案件</small></div>
...
const loadData = async () => {
  try {
    const res = await request.get('/lawyer/list', { params: { page: 1, limit: 30, keyword: keyword.value } })
    list.value = res.data?.list || []
  } catch (e) {}
}
const onClick = (item) => { router.push('/legal/consult?lawyerId=' + item.id) }
onMounted(loadData)
</script>
<style scoped>
