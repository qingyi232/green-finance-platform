<template>
  <div>
    <PageHeader title="我的咨询" parent="法律服务" />
...
onMounted(async () => {
  try { const res = await request.get('/legal-consult/mylist'); list.value = Array.isArray(res.data) ? res.data : (res.data?.list || []) } catch (e) {}
})
</script>
