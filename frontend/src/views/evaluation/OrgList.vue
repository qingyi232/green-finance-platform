<template>
  <div>
    <PageHeader title="评估机构列表" parent="资产评估" />
        ...
        <div v-for="item in list" :key="item.id" class="grid-card eco-card" @click="onClick(item)">
          <div class="gc-header">
            ...
            <div>
              ...
              <div class="gc-sub">{{ item.officeAddress }}</div>
            </div>
          </div>
          ...
          <div class="gc-stats">
            <span>{{ item.evaluatorCount }} 位评估师</span>
            <span>{{ item.caseCount }} 个案例</span>
...
const loadData = async () => {
  try {
    const res = await request.get('/evaluation-org/list', { params: { page: 1, limit: 30, keyword: keyword.value } })
    list.value = res.data?.list || []
  } catch (e) {}
}
const onClick = (item) => { router.push('/evaluation/apply?orgId=' + item.id) }
onMounted(loadData)
</script>
<style scoped>
