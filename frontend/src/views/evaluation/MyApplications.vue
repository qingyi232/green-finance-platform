<template>
  <div>
    <PageHeader title="我的评估申请" parent="资产评估" />
...
onMounted(async () => {
  try { const res = await request.get('/evaluation-apply/mylist'); list.value = Array.isArray(res.data) ? res.data : (res.data?.list || []) } catch (e) {}
})
</script>
